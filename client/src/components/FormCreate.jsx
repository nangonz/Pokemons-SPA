        ...
        <div>
            ...
            <form className={style.form} onSubmit={(e)=>handleOnSubmit(e)}>
                ...
                <fieldset>
                    ...
                    <div className={style.input}><label>Name:</label><span className={style.error}>{error.name}</span><input name="name" type='text' autoComplete="off" placeholder="Text only..." maxLength='12' onChange={(e)=> handleOnChange(e)}/></div>
                    
                    <div className={style.input}><label>Image:</label><span className={style.error}>{error.image}</span><input name="image" type="url" autoComplete="off" placeholder="image URL..." onChange={(e)=> handleOnChange(e)}/></div>
                    
                    <div className={style.input}><label>Weight:</label><span className={style.error}>{error.weight}</span><input type='number' name="weight" autoComplete='off' placeholder="less than 1000 kgs." min='1' onChange={(e)=> handleOnChange(e)}/></div>
                    
                    <div className={style.input}><label>Height:</label><span className={style.error}>{error.height}</span><input type='number' name="height" autoComplete="off" placeholder="lower than 10 fts." min='1' onChange={(e)=> handleOnChange(e)}/></div>
